        ...
        <div class="row mx-2">
            ...
            <div class="col-3 bg-light border-right">
                <ul class="nav nav-pills flex-column mb-3 border-bottom">
                    <li class="nav-item mb-1 container"><i class="fa fa-graduation-cap mr-2"></i>MIS CURSOS</li>
                    @foreach(auth()->user()->cursos()->where('anio',now()->year)->orderByDesc('created_at')->get() as $curso)
                        <li class="ml-2 nav-item "><a class="nav-link @if(Request::is('app/cursos/'.$curso->id)) active text-white @else text-dark @endif" href="{{route('app.cursos.show',$curso->id)}}">{{$curso->nombre . ' (' . $curso->anio . 'S' . $curso->semestre . ')'}}</a></li>
                    @endforeach
                    <li class="ml-2 nav-item "><a class="nav-link @if(Request::route()->getName() == "app.cursos.index") active text-white @else text-dark @endif" href="{{route('app.cursos.index')}}">Todos (Histórico)</a></li>
                </ul>
            </div>
            <!-- Content --->

    ...
    @isset($asignatura)
        ...
        <div class="card">
            <div class="card-header">
                <ul class="nav nav-tabs card-header-tabs">
                    <li class="nav-item"><a href="{{route('admin.asignaturas.show',$asignatura)}}" class="nav-link">Eventos</a></li>
                    <li class="nav-item"><a href="{{route('admin.asignaturas.participantes.show',$asignatura)}}" class="nav-link active">Participantes</a></li>
                </ul>
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-hover">
                        ...
                        @foreach($profesores as $profesor)
                            <tr class="table-info">
                                ...
                                <td class="text-center"><form action="{{ route('admin.asignaturas.participantes.destroy',['asignatura_id' => $asignatura->id, 'user_id' => $profesor->id]) }}" method="post">
                                        @csrf
                                        @method('delete')
                        ...
                        @foreach($estudiantes as $estudiante)
                            <tr>
                                ...
                                <td class="text-center"><form action="{{ route('admin.asignaturas.participantes.destroy',['asignatura_id' => $asignatura->id, 'user_id' => $estudiante->id]) }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <button type="submit" class="btn btn-danger">Remover</button>
                                    </form>
        ...
        <div class="modal fade" id="addParticipanteModal">
            <div class="modal-dialog">
                <form action="{{route('admin.asignaturas.participantes.store',$asignatura)}}" method="post">
